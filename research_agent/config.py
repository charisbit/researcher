    ...
    def get_model(self, agent_name: str) -> str:
        ...
        if use_local:
            ...
            if agent_name in self.config.get("agents", {}):
                return self.config["agents"][agent_name].get("local", 
                    self.config["defaults"]["local_model"])
            return self.config["defaults"]["local_model"]
        else:
            # 使用云端模型
            if agent_name in self.config.get("agents", {}):
                return self.config["agents"][agent_name].get("cloud",
                    self.config["defaults"]["cloud_model"])
